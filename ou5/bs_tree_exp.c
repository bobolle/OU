...
int main(void)
{
    ...
    int start_no_of_val = 10; /* The number of values in the smallest tree. */
    int end_no_of_val = 10000;  /* The number of values in the largest tree. */
    int inc_no_of_val = 10;   /* The size of increments of values. */
    int no_of_repeats = 5;    /* The number of runs for each size of the tree. */
    srand(time(0));
    ...
    for (int n = start_no_of_val ; n <= end_no_of_val ; n += inc_no_of_val) {
        ...
        for (int experiment = 0 ; experiment < no_of_repeats ; experiment++) {
            ...
            /* Create a binary search tree and insert the values in the 
            array. */
            ...
            T = get_time() - T;
            T /= n;  /* Compute average time per searched 
            value. */
...
void insert_value(int value, BSTreePos first_pos) {
    BSTreePos parent = first_pos;
    while (first_pos != NULL) {
        parent = first_pos;
        if (value < first_pos->value) {
            first_pos = bs_tree_left_child(first_pos);
        }
        else if (value > first_pos->value) {
